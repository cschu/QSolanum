...
p1.date, p1.number AS FW_abs_brutto_g,
p2.date, p2.number AS DW_abs_g,
p3.date, p3.number AS tara_abs_g,

pt.value_id AS treatment,
p1.number >= p3.number AS sanity1_passed,
(p1.number - p3.number) / p2.number AS FW_DW,
0 <= (p1.number - p3.number) / p2.number <= 15 AS sanity2_passed
...
JOIN phenotype_plants AS pp3 ON pp3.plant_id = p.id
...
p2.date <= (p1.date + INTERVAL 7 DAY) AND

p3.date >= (p1.date - INTERVAL 10 DAY) AND
p3.date <= (p1.date - INTERVAL 1 DAY)
