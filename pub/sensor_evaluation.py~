...
def main(argv):
    ...
    for year in [2011, 2012, 2013]:        
        start, floweringTimes[year], end = weather.FIELD_TRIALS[(year, 'MPI')]
        floweringTimes[year] = weather.computeFloweringWeek(floweringTimes[year])
        rawTemperatures[year], stuff = map(lambda x:ff(x, start_date=start, end_date=end),
                                           weather.read_mpi_climate_data(weather.mpi_climate[year]))
        groupedTemperatures[year] = weather.pack_ungrouped_data(rawTemperatures[year], key=0)
        maxTemperatures[year], minTemperatures[year] = (
            weather.aggregate_values_from_grouped(groupedTemperatures[year], max),
            weather.aggregate_values_from_grouped(groupedTemperatures[year], min))
        ...
        weather.add_plot(figure, HEATSUM, SWC, figfn, figtitle, [year],
    ...
    rawTemperatures['GolmCGW'], stuff = weather.read_mpi_climate_data(weather.golm_cgw)
    sp_offset = 2
    subplots = [334, 335, 337, 338]
    figlabels = ['D', 'E', 'G', 'H']
    figfn='MPICGW'
    for trial, tdata in sorted(weather.CGW_TRIALS.items()):
        floweringTimes[trial] = weather.computeFloweringWeek(tdata[1])
        rawTemperatures[trial] = ff(rawTemperatures['GolmCGW'], start_date=tdata[0], end_date=tdata[2])
        groupedTemperatures[trial] = weather.pack_ungrouped_data(rawTemperatures[trial], key=0)
        maxTemperatures[trial], minTemperatures[trial] = (
            weather.aggregate_values_from_grouped(groupedTemperatures[trial], max),
            weather.aggregate_values_from_grouped(groupedTemperatures[trial], min))
        ...
        weather.add_plot(figure, HEATSUM, SWC, figfn, figtitle, [trial],
                         floweringTimes, suffix='', subplot=subplot, figlabel=figlabels.pop(0),
                         greyscale=GREYSCALE,
                         xlabel='Heatsum [$\mathbf{^\circ}\mathbf{C_{cumul}}$]', 
                         ylabel='SWC [$\mathbf{\%nFC}$]',
    ...
    for year in [2012, 2013]:
        start, floweringTimes[year], end = weather.FIELD_TRIALS[(year, 'JKI')]
        floweringTimes[year] = weather.computeFloweringWeek(floweringTimes[year])
        rawTemperatures[year], stuff = map(lambda x:ff(x, start_date=start, end_date=end),
                                           weather.read_jki_climate_data(weather.jki_climate[year]))
        groupedTemperatures[year] = weather.pack_ungrouped_data(rawTemperatures[year], key=0)
        maxTemperatures[year], minTemperatures[year] = (
            weather.aggregate_values_from_grouped(groupedTemperatures[year], max),
            weather.aggregate_values_from_grouped(groupedTemperatures[year], min))
    ...
    """
    HEATSUM = {year: climate.compute_heatsum_per_week(climate.compute_heatsum_per_day(
                maxTemperatures[year], minTemperatures[year]))
               for year in set(maxTemperatures.keys()).intersection(set(minTemperatures.keys()))}
    """
    weather.add_plot(figure, HEATSUM, SWC, figfn, figtitle, [2011, 2012, 2013], 
                     floweringTimes, suffix='', subplot=311, figlabel='', greyscale=GREYSCALE,
                     xlabel='Heatsum [$\mathbf{^\circ}\mathbf{C_{cumul}}$]', 
                     ylabel='Rel Soil Moisture [$\%$]')
                     #ylabel='Soil Water Content [$\mathbf{\%nFC}$]')
